;;; ffap-urls-menu.el --- Interface for viewing and choosing URLs to open. -*- lexical-binding: t -*-
...
;; (`ffap-urls-menu-list-urls').

;;; Code:

(require 'tabulated-list)

(defgroup ffap-urls-menu nil
  "Show a menu of all visible URLs in a buffer."
  :group 'tools
  ;; TODO: :group 'communication or something
  :group 'convenience)

(defvar ffap-urls-menu-marker-char ?>
...
(defcustom ffap-urls-menu-name-width #'ffap-urls-menu--dynamic-name-width
  "Width of buffer name column in the FFAP URLs Menu.
...
(defun kakafarm/ffap-browse-urls ()
  ...
  (let* ((urls (mapcar 'car (ffap-menu-rescan)))
         (urls-to-open (completing-read-multiple "Which URLs should I open? "
                                                 urls)))
...
;;;###autoload
(defun kakafarm/list-urls (&optional arg)
  "Display a list of visible URLs.
The list is displayed in a buffer named \"*URL List*\".
...
;;;###autoload
(defun kakafarm/list-urls-noselect (&optional files-only buffer-list filter-predicate)
  "Create and return a URLs Menu (what a lame name.  I wanted it to be based on ffap-scan-thingamajig) buffer.
This is called by `buffer-menu' and others as a subroutine.
...
See more at `Buffer-menu-filter-predicate'."
  (let ((old-buffer (current-buffer))
	(buffer (get-buffer-create "*URL List*")))
    (with-current-buffer buffer
      (URLs-menu-mode)
      (setq URLs-menu-files-only
            ;; TODO: flag to show only file buffers originally but here maybe only local file URLs?
            (and files-only
                 (>= (prefix-numeric-value files-only)
                     0)))
      (setq URLs-menu-buffer-list buffer-list)
      (setq URLs-menu-filter-predicate filter-predicate)
      (list-URLs--refresh buffer-list old-buffer)
      (tabulated-list-print))
    buffer))

(defun kakafarm/ffap--refresh (&optional buffer-list old-buffer)
  ;; Set up `tabulated-list-format'.
  (let* ((urls (mapcar 'car (ffap-menu-rescan)))
         (size-width kakafarm/ffap-size-width)
         (marked-urls (kakafarm/ffap-marked-buffers))
         (ffap-buffer (current-buffer))
	 (show-non-file (not kakafarm/ffap-files-only))
	 (filter-predicate (and (functionp kakafarm/ffap-filter-predicate)
			        kakafarm/ffap-filter-predicate))
	 entries name-width)
    ;; Collect info for each buffer we're interested in.
    (dolist (buffer (cond
                     ((functionp buffer-list)
                      (funcall buffer-list))
                     (buffer-list)
                     ((functionp kakafarm/ffap-buffer-list)
                      (funcall kakafarm/ffap-buffer-list))
                     (kakafarm/ffap-buffer-list)
                     (t (buffer-list
                         (if kakafarm/ffap-use-frame-buffer-list
                             (selected-frame))))))
      (with-current-buffer buffer
	(let* ((name (buffer-name))
	       (file buffer-file-name))
	  (when (and (buffer-live-p buffer)
	    ...
	    (push (list buffer
			(vector (cond
				...
				(if buffer-read-only "%" " ")
				(if (buffer-modified-p) "*" " ")
				(kakafarm/ffap--pretty-name name)
				(number-to-string (buffer-size))
				(concat (format-mode-line mode-name
                                                          nil nil buffer)
					(if mode-line-process
					    (format-mode-line mode-line-process
							      nil nil buffer)))
				(kakafarm/ffap--pretty-file-name file)))
		  entries)))))
    (setq name-width (if (functionp kakafarm/ffap-name-width)
                         (funcall kakafarm/ffap-name-width (mapcar #'car entries))
                       kakafarm/ffap-name-width))
    (setq tabulated-list-format
	  (vector '("C" 1 t :pad-right 0)
		  '("R" 1 t :pad-right 0)
		  '("M" 1 t)
		  ...
		  `("Size" ,size-width tabulated-list-entry-size->
                    :right-align t)
		  `("Mode" ,kakafarm/ffap-mode-width t)
		  '("File" 1 t)))
    (setq tabulated-list-use-header-line kakafarm/ffap-use-header-line)
    (setq tabulated-list-entries (nreverse entries)))
  (tabulated-list-init-header))

(defun kakafarm/ffap-list-urls ()
  (interactive)

  (let ((old-buffer (current-buffer))
        (buffer (get-buffer-create "*FFAP URLs*")))
    (with-current-buffer buffer
